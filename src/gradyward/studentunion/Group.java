...
public class Group {
	...
	public boolean addMembers(List<String> toAddEmails){
		if (User.loggedIn()){
			...
			if (p != null && (p.owner || User.isGoogleAdmin() || leaders.contains(p.email))){
				...
				if (newMembers.size() != members.size()){
					...
					Log.info(String.format("User [%s] added [%s] as members to group [%s].\n", p.email, toAddEmails.toString(), this.name));
					return true;
				}
				return false;
			}
			Log.warn(String.format("User [%s] attempted to add [%s] as members to group [%s] but was prevented because they didn't have sufficent permissions.", p.email, toAddEmails.toString(), this.name));
			return false;
		}
		return false;
	}
		...
		if (User.loggedIn()){
			...
			if (p != null && (p.owner || User.isGoogleAdmin() || leaders.contains(p.email))){
				...
				if (newMembers.size() != members.size()){
					this.members = newMembers;
					ofy.save().entity(this);
					Log.info(String.format("User [%s] removed [%s] as members from group [%s].\n", p.email, toRemoveEmails.toString(), this.name));
					return true;
				}
				return false;
			}
			Log.warn(String.format("User [%s] attempted to remove [%s] as members of group [%s] but was prevented because they didn't have sufficent permissions.", p.email, toRemoveEmails.toString(), this.name));
			return false;
		}
		return false;
	}
		...
		if (User.loggedIn()){
			...
			if (p != null && (p.owner || User.isGoogleAdmin() || leaders.contains(p.email))){
				...
				if (newLeaders.size() != leaders.size()){
					...
					ofy.save().entity(this);
					Log.info(String.format("User [%s] added [%s] as leaders to group [%s].\n", p.email, newLeaders.toString(), this.name));
					return true;
				}
				return false;
			}
			Log.warn(String.format("User [%s] attempted to add [%s] as leaders to group [%s] but was prevented because they didn't have sufficent permissions.", p.email, toAddEmails.toString(), this.name));
			return false;
		}
		return false;
	}
		...
		if (User.loggedIn()){
			...
			if (p != null && (p.owner || User.isGoogleAdmin() || leaders.contains(p.email))){
				...
				if (newLeaders.size() != leaders.size()){
					this.leaders = newLeaders;
					ofy.save().entity(this);
					Log.info(String.format("User [%s] removed [%s] as leaders from group [%s].\n", p.email, toRemoveEmails.toString(), this.name));
					return true;
				}
				return false;
			}
			Log.warn(String.format("User [%s] attempted to remove [%s] as leaders of group [%s] but was prevented because they didn't have sufficent permissions.", p.email, toRemoveEmails.toString(), this.name));
			return false;
		}
		return false;
	}
